...
export function Header(){return <header className="site-header"><div className="shell nav-wrap"><Brand/><nav aria-label="Primary navigation">{nav.map(([n,h])=><Link key={h} href={h}>{n}</Link>)}<Link className="nav-contact" href="/contact">Contact ↗</Link></nav><details className="mobile-nav"><summary aria-label="Open navigation"><span/><span/></summary><div>{nav.map(([n,h])=><Link key={h} href={h}>{n}</Link>)}<Link href="/contact">Contact ↗</Link></div></details></div></header>}
