...
- Create proposal list — in progress: list supports search, status filters, proposal metadata, dev mock labels, direct links into proposal detail and a local mock create-proposal draft flow gated by observed readiness state
...
- Evolve Governance Operations Center — in progress: `/governance/console` is positioned as a federation observability surface, not a narrow DAO admin page, with operations readiness visibility for registry, selected context, proposals, plugins and proposal creation state
