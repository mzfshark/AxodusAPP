...
- Create proposal list — in progress: list supports search, status filters, proposal metadata, dev mock labels, direct links into proposal detail and local proposal drafts persisted as browser-only observed state with request inspection, review/submit states, local mock receipts, backend submission error/retry display with backend reason metadata, mode-aware submit labels, modal-level integration state, and an env-gated backend createProposal submission contract whose preview request mirrors the active integration mode
...
- Evolve Governance Operations Center — in progress: `/governance/console` is positioned as a federation observability surface, not a narrow DAO admin page, with operations readiness visibility for registry, selected context, proposals, plugins, local drafts, proposal creation state and a reusable createProposal integration status panel
