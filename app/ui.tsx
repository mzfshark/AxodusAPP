...
export function PublicationCard({title,type,status,audience,date,summary,href}:{title:string,type:string,status:string,audience:string,date:string,summary:string,href?:string}) {
  return <article className="publication-card"><div className="pub-meta"><span>{type}</span><StatusBadge>{status}</StatusBadge></div><h3>{title}</h3><p>{summary}</p><dl><div><dt>Audience</dt><dd>{audience}</dd></div><div><dt>Date</dt><dd>{date}</dd></div></dl>{href?<a className="pub-link" href={href}>Document overview <b>↗</b></a>:<span className="pub-link muted">Publication in preparation</span>}</article>;
}
...
export function Footer(){return <footer><div className="shell footer-main"><div><Brand footer/><p>A governed prototype for a new ecosystem direction.</p></div><div><h3>Explore</h3>{nav.slice(0,3).map(([n,h])=><Link key={h} href={h}>{n}</Link>)}</div><div><h3>Project</h3>{nav.slice(3).map(([n,h])=><Link key={h} href={h}>{n}</Link>)}<Link href="/contact">Contact</Link></div></div><div className="shell footer-disclaimer"><p>Axodus is currently in prototype stage. Public materials are informational and directional; they do not represent a production-ready platform, operational guarantee, investment solicitation, legal offer or promise of financial performance.</p><span>© 2026 Axodus · Institutional website</span></div></footer>}
