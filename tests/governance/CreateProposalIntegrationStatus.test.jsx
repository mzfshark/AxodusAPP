...
async function renderPanel({ backendEnabled = false, proposalDrafts = [], reviewRequestsState } = {}) {
  vi.resetModules();
  ...
  return render(<CreateProposalIntegrationStatus proposalDrafts={proposalDrafts} reviewRequestsState={reviewRequestsState} />);
}
...
describe('CreateProposalIntegrationStatus', () => {
  ...
  test('renders backend integration state without frontend reason codes', async () => {
    ...
    expect(screen.getByText(/0 observed review requests/i)).toBeInTheDocument();
    expect(screen.getByText(/no backend createProposal review requests are currently observed/i)).toBeInTheDocument();
  });

  test('renders observed backend review requests', async () => {
    await renderPanel({
      backendEnabled: true,
      reviewRequestsState: {
        status: 'success',
    ...
    expect(screen.getByText(/1 observed review request/i)).toBeInTheDocument();
