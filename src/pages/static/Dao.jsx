...
const Dao = () => {
  return (
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-16 bg-[#131b2e]/80 backdrop-blur-xl flex justify-between items-center px-6 border-b border-outline-variant/10">
        <div className="flex items-center gap-8">
          <div className="text-2xl font-black tracking-tighter text-[#c0c1ff] flex items-center gap-2">
            <span className="material-symbols-outlined text-primary" style={{fontVariationSettings: "'FILL' 1"}}>account_balance</span>
            Axodus
          </div>
        </div>
        {/* Centered Search */}
        <div className="absolute left-1/2 -translate-x-1/2 hidden md:flex items-center bg-surface-container-lowest px-4 py-1.5 rounded-full border border-outline-variant/10">
          <span className="material-symbols-outlined text-[#908fa0] text-sm" data-icon="search">search</span>
          <input className="bg-transparent border-none text-sm focus:ring-0 text-on-surface w-64 placeholder-[#908fa0]" placeholder="Search governance..." type="text"/>
        </div>
        <div className="flex items-center gap-6">
          <div className="hidden sm:flex flex-col items-end px-4 border-r border-outline-variant/20">
            <span className="text-[10px] uppercase font-bold text-[#908fa0]">$NEURONS</span>
            <span className="text-xs font-bold text-[#c0c1ff]">$0.4285</span>
          </div>
          <div className="flex items-center gap-4">
            <span className="material-symbols-outlined text-[#908fa0] hover:text-[#c0c1ff] cursor-pointer transition-colors duration-200" data-icon="notifications">notifications</span>
            <span className="material-symbols-outlined text-[#908fa0] hover:text-[#c0c1ff] cursor-pointer transition-colors duration-200" data-icon="settings">settings</span>
            <button className="primary-gradient text-on-primary text-sm font-bold px-5 py-2 rounded-lg active:scale-95 transition-all">
              Connect Wallet
            </button>
          </div>
        </div>
      ...
      <div className="flex pt-16 min-h-screen">
        {/* SideNavBar (Below Header) */}
        <aside className="fixed left-0 top-16 h-[calc(100vh-64px)] w-64 hidden lg:flex flex-col bg-[#0b1326] border-r border-outline-variant/10 z-40">
          <div className="flex flex-col p-4 h-full">
            <nav className="flex-1 space-y-1 overflow-y-auto scrollbar-hide">
              <a href="index.html" className="flex items-center gap-3 px-4 py-2.5 text-[#908fa0] hover:text-white hover:bg-[#131b2e] transition-all duration-300 font-medium text-sm">
                <span className="material-symbols-outlined" data-icon="dashboard">dashboard</span> Overview
              </a>
              <a href="mining.html" className="flex items-center gap-3 px-4 py-2.5 text-[#908fa0] hover:text-white hover:bg-[#131b2e] transition-all duration-300 font-medium text-sm">
                <span className="material-symbols-outlined" data-icon="memory">memory</span> Mining
              </a>
              <a href="tradingbots.html" className="flex items-center gap-3 px-4 py-2.5 text-[#908fa0] hover:text-white hover:bg-[#131b2e] transition-all duration-300 font-medium text-sm">
                <span className="material-symbols-outlined" data-icon="swap_horiz">swap_horiz</span> Trading
              </a>
              <a href="defi.html" className="flex items-center gap-3 px-4 py-2.5 text-[#908fa0] hover:text-white hover:bg-[#131b2e] transition-all duration-300 font-medium text-sm">
                <span className="material-symbols-outlined" data-icon="lock_open">lock_open</span> Staking
              </a>
              <a href="dao.html" className="flex items-center gap-3 px-4 py-2.5 bg-[#2d3449] text-[#c0c1ff] rounded-lg shadow-[0_0_15px_rgba(192,193,255,0.1)] font-medium text-sm">
                <span className="material-symbols-outlined" data-icon="account_balance" style={{fontVariationSettings: "'FILL' 1"}}>account_balance</span> DAOs
              </a>
              <a href="business.html" className="flex items-center gap-3 px-4 py-2.5 text-[#908fa0] hover:text-white hover:bg-[#131b2e] transition-all duration-300 font-medium text-sm">
                <span className="material-symbols-outlined" data-icon="business_center">business_center</span> Business
              </a>
              <a href="marketplace.html" className="flex items-center gap-3 px-4 py-2.5 text-[#908fa0] hover:text-white hover:bg-[#131b2e] transition-all duration-300 font-medium text-sm">
                <span className="material-symbols-outlined" data-icon="shopping_basket">shopping_basket</span> Marketplace
              </a>
              <a href="academy.html" className="flex items-center gap-3 px-4 py-2.5 text-[#908fa0] hover:text-white hover:bg-[#131b2e] transition-all duration-300 font-medium text-sm">
                <span className="material-symbols-outlined" data-icon="school">school</span> Academy
              </a>
              <a href="defi.html" className="flex items-center gap-3 px-4 py-2.5 text-[#908fa0] hover:text-white hover:bg-[#131b2e] transition-all duration-300 font-medium text-sm">
                <span className="material-symbols-outlined" data-icon="payments">payments</span> Lending
              </a>
              <a href="dex.html" className="flex items-center gap-3 px-4 py-2.5 text-[#908fa0] hover:text-white hover:bg-[#131b2e] transition-all duration-300 font-medium text-sm">
                <span className="material-symbols-outlined" data-icon="currency_exchange">currency_exchange</span> DEX
              </a>
              <a href="lottery.html" className="flex items-center gap-3 px-4 py-2.5 text-[#908fa0] hover:text-white hover:bg-[#131b2e] transition-all duration-300 font-medium text-sm">
                <span className="material-symbols-outlined" data-icon="confirmation_number">confirmation_number</span> Lottery
              </a>
              <a href="mcps.html" className="flex items-center gap-3 px-4 py-2.5 text-[#908fa0] hover:text-white hover:bg-[#131b2e] transition-all duration-300 font-medium text-sm">
                <span className="material-symbols-outlined" data-icon="dns">dns</span> MCP Servers
              </a>
            </nav>
            <div className="mt-auto p-4 bg-surface-container rounded-xl">
              <div className="flex items-center gap-3 mb-4">
                <img alt="User Profile" className="w-8 h-8 rounded-full bg-surface-container-highest" src="https://lh3.googleusercontent.com/aida-public/AB6AXuB3dM6nvIG5cjj0yUjACMSFkjdHtebpEjaH7whUd2p8my4Da36kYvvSNlbHLyttJVc2VjGIbyd-o6neHF3ZsC0J8ou5IUJTDO0PzQM_UuyQouXhfIjZnHwR8chhv8x2vAHfAOiTrkT4Yq0upwwVmPl2GjXBVv04xoPz1mF-XPa_kuG7pZVq1mIe0fpTDmJvJaB5oxCQXMYu1SIo9pm3OXtBHbBMOOKfjSJfpSDUmTzU_xTmzj9EPA9KxcsFt9e7xB31zzgCv_oH2Xc"/>
                <div>
                  <div className="text-white text-xs font-bold leading-none">Axodus</div>
                  <div className="text-[#908fa0] text-[10px] mt-1">Kinetic Observatory</div>
                </div>
              </div>
              <button className="w-full primary-gradient text-on-primary py-2.5 rounded-lg text-xs font-bold active:scale-95 transition-transform">
                Launch Terminal
              </button>
            </div>
          </div>
              ...
              <div className="glass-panel rounded-2xl p-6 border border-outline-variant/5 hover:border-primary/20 transition-all group">
                <div className="flex justify-between items-start mb-6">
                  <div className="space-y-1">
                    ...
                    <h3 className="text-lg font-bold text-on-surface leading-snug group-hover:text-primary transition-colors">Liquidity Mining Rewards Optimization for Q3</h3>
                  </div>
                ...
                <p className="text-sm text-on-surface-variant mb-8 line-clamp-2">Adjusting the distribution of $NEURONS rewards across main DEX pools to prioritize deep liquidity in stable pairs and sustainable growth in MCP Server nodes.</p>
                {/* Voting Bars */}
              ...
              <div className="glass-panel rounded-2xl p-6 border border-outline-variant/5 hover:border-primary/20 transition-all group">
                <div className="flex justify-between items-start mb-6">
                  <div className="space-y-1">
                    ...
                    <h3 className="text-lg font-bold text-on-surface leading-snug group-hover:text-primary transition-colors">Expansion into Arweave Storage Network</h3>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] text-outline uppercase font-bold">Ends In</div>
                ...
                <p className="text-sm text-on-surface-variant mb-8 line-clamp-2">Integrating Arweave to permanently store DAO governance logs and MCP server metadata for verifiable transparency across the Kinetic Observatory.</p>
                {/* Voting Bars */}
                <div className="space-y-4">
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs font-bold">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-[#131b2e]/90 backdrop-blur-xl flex justify-around items-center h-16 z-50 border-t border-outline-variant/10">
        <a className="flex flex-col items-center gap-1 text-[#908fa0]" href="#">
          <span className="material-symbols-outlined" data-icon="home">home</span>
          <span className="text-[10px]">Home</span>
        </a>
        <a className="flex flex-col items-center gap-1 text-primary" href="#">
          <span className="material-symbols-outlined" data-icon="account_balance" style={{fontVariationSettings: "'FILL' 1"}}>account_balance</span>
